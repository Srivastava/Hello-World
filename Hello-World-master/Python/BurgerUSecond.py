...
def evolve(T,Ti):
    ...
    '''k1[2:-1]=-0.25*(3*Ti[2 :-1 ]*Ti[2 :-1 ]-4*Ti[1:-2]*Ti[1:-2]+Ti[ :-3 ]*Ti[ :-3 ])/(dx)
    Ti[2:-1]=Ti[2:-1]+(8.0/15.0)*dt*k1[2:-1]

    k2[2:-1]=-0.25*(3*Ti[2 :-1 ]*Ti[2 :-1 ]-4*Ti[1:-2]*Ti[1:-2]+Ti[ :-3 ]*Ti[ :-3 ])/(dx)
    Ti[2:-1]=Ti[2:-1]+(5.0/12.0)*dt*k2[2:-1]-(17.0/60.0)*dt*k1[2:-1]

    k3[2:-1]=-0.25*(3*Ti[2 :-1 ]*Ti[2 :-1 ]-4*Ti[1:-2]*Ti[1:-2]+Ti[ :-3 ]*Ti[ :-3 ])/(dx)
    Ti[2:-1]=Ti[2:-1]+(3.0/4.0)*dt*k3[2:-1]-(5.0/12.0)*dt*k2[2:-1]'''
    Ti[2:-1]=Ti[2:-1]-0.25*dt*(3*Ti[2 :-1 ]*Ti[2 :-1 ]-4*Ti[1:-2]*Ti[1:-2]+Ti[ :-3 ]*Ti[ :-3 ])/(dx)
    #Ti[0]=1
    #Ti[nx-1]=0
...
for m in range(0, timeStep+1):
    ...
    plt.title('Burgers Equation using RK3 in time and 2nd Order Upwind in Space')

    if(timeEl==0 or timeEl==0.5 or timeEl==1 or timeEl==1.5 or  timeEl==5 or timeEl==10):
        points.set_data(x,Ti)

        fig.savefig('FigureBurgerUVSe'+str(timeEl)+'.png',dpi=600)
